...
<div class="text-white no-underline card bg-facebook bg-facebook-favorites mb-3">
    <div class="card-body d-flex justify-content-between">
        <div>
            <i class="<?= $language->{$source}->global->icon ?>"></i> <?= sprintf($language->favorites->source_header, $language->{$source}->global->name) ?>
        </div>
...
<table class="table table-responsive-lg mt-3">
    ...
    <?php foreach($source_users as $source_account): ?>
        <tr>
            <td><a href="report/<?= $source_account->username ?>/<?= $source ?>"><?= $source_account->username ?></a></td>
