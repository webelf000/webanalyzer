...
if($type && $type == 'delete') {
    ...
    if(empty($_SESSION['error'])) {
        ...
        $database->query("DELETE FROM `twitter_tweets` WHERE `twitter_user_id` = {$twitter_user_id}");
        ...
        $database->query("DELETE FROM `email_reports` WHERE `source_user_id` = '{$twitter_user_id}' AND `source` = 'TWITTER'");
